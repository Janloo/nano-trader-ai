    ...
    <script>
        ...
        async function loadPerformanceChart(asset) {
            try {
                ...
                const realLabels = data.real.curve.map(p => p.x.substring(11, 16));
                const shadowLabels = data.shadow.curve.map(p => p.x.substring(11, 16));
                
                const labels = realLabels.length > shadowLabels.length ? realLabels : shadowLabels;
                
                const realData = data.real.curve.map(p => p.y);
                const shadowData = data.shadow.curve.map(p => p.y);
