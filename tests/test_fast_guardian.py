...
class TestFastGuardian(unittest.TestCase):
    def setUp(self):
        # We mock the Gemini client so we don't make real API calls in CI
        self.guardian = FastGuardian()
        self.guardian.client = MagicMock()

    def test_cataclysm_evaluation(self):
        mock_response = MagicMock()
        mock_response.text = "CATACLYSM"
        self.guardian.client.models.generate_content.return_value = mock_response
